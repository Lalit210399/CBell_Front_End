...
export const TaskStatusProvider = ({ children }) => {
  ...
  const fetchTaskStatuses = useCallback(async (forceRefresh = false) => {
    ...
    if (!forceRefresh && taskStatuses.length > 0 && lastFetched) {
      const now = Date.now();
      if (now - lastFetched < CACHE_DURATION) {
        return taskStatuses; // Return cached data
      }
    }
      ...
      if (!response.ok) {
        if (response.status === 404) {
          ...
          const fallbackStatuses = [
            { id: '1', statusName: 'Draft', name: 'Draft', isActive: true, color: 'gray' },
            { id: '2', statusName: 'In Progress', name: 'In Progress', isActive: true, color: 'blue' },
            { id: '3', statusName: 'Pending', name: 'Pending', isActive: true, color: 'orange' },
            { id: '4', statusName: 'Completed', name: 'Completed', isActive: true, color: 'green' },
            { id: '5', statusName: 'Cancelled', name: 'Cancelled', isActive: true, color: 'red' },
            { id: '6', statusName: 'On Hold', name: 'On Hold', isActive: true, color: 'yellow' }
          ];
  ...
  }, [selectedOrganizationId, user?.organizationId, user?.userId, lastFetched, CACHE_DURATION, taskStatuses.length]);
  ...
  }, [selectedOrganizationId, fetchTaskStatuses, user?.organizationId, user?.userId, user]);
